...
/// <summary>
/// Writes the converter for one pair, or nothing when the pair needs none. The class around the body
...
internal sealed class ConverterEmitter
{
    ...
    public IEnumerable<GeneratedSource> Emit(ConversionPair pair)
    {
        ...
        if (body == null)
        {
            // Nobody writes this pair. The library answers some of them itself - a string read back
            // into a scalar it has a built-in converter for - and for the rest there is nothing
            // left: the pair was asked for, no converter was written by hand, and no rule knows how
            // to write one. The call would compile and throw at runtime, so it fails the build.
            if (_semantics.IsBuiltInStringConversion(pair.From, pair.To))
            {
                yield break;
            }

            throw new ConversionException(
                ConverterDiagnostics.ConversionWithoutConverter,
                pair.From,
                pair.To);
        }

...
public class ImplementedConvertersTest : ConversionTestBase<ImplementedConvertersComposition>
{
    ...
    [TestMethod]
    public void WhenAnUnwritablePairIsWrittenThenNothingIsReported()
    {
        AssertGeneratedWithoutDiagnostics();

        var converted = GetConvertingService().Convert<int, long>(5);

        Assert.AreEqual(NumberConverter.Marker + 5, converted);
    }
}

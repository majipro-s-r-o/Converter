...
A rule that recognizes a conversion but finds it impossible to write deterministically throws `Diagnostics/ConversionException`, which carries the `DiagnosticDescriptor` it is to be reported as. `ConverterGenerator` catches it ahead of the catch all and reports that descriptor instead of `MC0001`. Both are `DiagnosticSeverity.Error`, so either one fails the build, and both end the whole generation pass — the pairs after the failing one are never travelled. That is deliberate: the pair has to be dealt with by hand before anything else is worth generating.
...
`Conversions/` mirrors the reference project's `Generator.Tests/TestCases/Basic`: `PropertyOfTheSameType` (the baseline), `DifferentSourcesAndTargets` (asymmetric property sets), `ObjectNesting` (nested converters and collection properties), `AccessModifiers` (private, `init` and get only properties), `FileScopedNamespace` (types declared straight in a file scoped namespace instead of nested in a test case class), `Structures` (structs, `Guid -> Guid?`, differing nullable annotations), `NullableTypes` (`Nullable<T>` on either side of the call site and `string` targets) `CollectionProperties` (every collection type `GetListItemType` accepts as a target, an array as the source, and items that are the same type on both sides), `Enums` (`enum -> string`, an enum property copied to the same enum or its nullable counterpart, and two different enum types being left unmapped as a *property*) and `EnumConversion` (`enum -> enum` call sites: the same members on both sides, source members that are a subset of the target's and numbered differently, and a nullable enum on either side).

`Validations/` holds the two shapes `MC0002` refuses: `MissingEnumMember` (the target has no counterpart for one source member) and `DisjointEnumMembers` (neither enum covers the other, which being a subset in the *other* direction does not help). They are two cases rather than two compositions of one case because an `MC0002` ends the whole generation pass, so each refused pair needs a compilation of its own.
...
The solution builds clean and both suites pass (135 + 59). Open items:
- `enum -> enum` is generated for a **call site** only. As a *property* type it is still dropped silently: `ConvertedValueRule` gates on `IsMappableSource`, which is class-or-struct, so an enum property never reaches `EnumBodyRule`. Letting it through is a one line change, but it turns today's silent drop into a hard `MC0002` for every pair of enums in a consuming project that does not line up — worth doing deliberately, not as a side effect.
